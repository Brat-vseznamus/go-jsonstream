...
func readFloat(props *NumberProps) (f float64, n int, err error) {
	...
	if !props.trunc {
		if f, ok := atof64exact(props.mantissa, props.exponent, props.isNegative); ok {
			return f, n, nil
		}
	}
	f, ok = eiselLemire64(props.mantissa, props.exponent, props.isNegative)
	if ok {
		if !props.trunc {
			return f, n, nil
		}
		...
		fUp, ok := eiselLemire64(props.mantissa+1, props.exponent, props.isNegative)
		if ok && f == fUp {
			return f, n, nil
		}
	}
	...
	// Slow fallback.
	f, err = strconv.ParseFloat(string(props.raw), 64)
	return
}
